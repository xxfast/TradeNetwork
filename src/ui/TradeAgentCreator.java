...
public class TradeAgentCreator extends JDialog implements ActionListener {
	...
	private JPanel CreateInspector(Class<?> type, Object instance, JPanel container)
		...
		for (Field f : type.getDeclaredFields()) {
			...
			if (f.isAnnotationPresent(Adjustable.class)) {
				...
				if (innerType.isAnnotationPresent(Adjustable.class)) {
					...
					String labelText = ((c.label().equals("")) ? UIUtilities.ProcessVariableName(f.getName())
							: c.label());
					JLabel label = new JLabel(labelText, JLabel.RIGHT);
					labels.add(label);
					JComponent input;
					/* Checks if the type refers to another Agent */
					if (f.getType() != AID.class && !f.getType().isEnum()) {
						...
						if (!f.isAnnotationPresent(Nullable.class)) {
							try {
								...
								if (fd != null)
									((JTextField) input).setText(fd.toString());
							} catch (InvocationTargetException | IntrospectionException | IllegalAccessException e) {
		...
		for (int i = 0; i < inpectors.size(); i++) {
			inpectors.get(i).setBorder(
					BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), inspectorLabel.get(i)));
	...
	private void Validate(Object instance, Class<?> whatToValidate, Field toValidate, JComponent toShow) {
		...
		try {
			...
			if (primitiveRegistry.containsKey(desiredType))
				desiredType = primitiveRegistry.get(desiredType);
			...
			} else if(toShow instanceof JComboBox<?>) {
				Object value = ((JComboBox) toShow).getSelectedItem();
				if (!value.equals(null)) {
					Object fd = setter.invoke(instance, value);
					return;
				}
